...
where \highlighttext{one relaxes} regularity.
...
a term now commonly used. \highlighttext{This paper} reserves \emph{BC helix} to mean the purely
...
flat planar surface. By making length changes in a certain way, a \highlighttext{tetrahelix untwists} to form a \emph{tetrabeam} which has planar faces
...
\Cref{fig:series} displays a continuum of tetrahelices optimal in a \highlighttext{in the sense of being
...
\highlighttext{one} can dynamically untwist a physical Tetrobot
forming the Boerdijk-Coxeter configuration into the equitetrabeam which rests flat on the plane,
as shown in figs. \highlighttext{4 and} 5.

By untwisting the tetrahelix so that is has a planar surface resting on the ground,
each vertex touching the ground \highlighttext{becomes} a foot or pseudopod. A robot can thus
...
\highlighttext{Engineers} would like to design nearly regular tetrahelices with a formula that
gives the vertices in space. Eventually, \highlighttext{this would be done}
...
Ideally, \highlighttext{a simple formula would define} the vertices based on
...
\highlighttext{a formula is needed}
...
$h$ is used for one third of the distance we name $d$. \highlighttext{The fact that $d = 3h$ is justified later}.
...
Intuitively, an expression where $c/3$ is multiplied by a $1/3$ rotation plus the rail angle $\rho$ \highlighttext{is sought}.
We expand 
...
\begin{align*}
  (c/3)  (3 \theta_{bc}) 
\end{align*}
\highlighttext{seeking} $2\pi$ in the numerator, add canceling terms...
\begin{align*}
...
\end{align*}
\highlighttext{to} finally obtain:
\begin{equation}
  c  ( \rho_{bc} +  2 \pi) /3  \text{.}
\end{equation}  
...
\highlighttext{It can now be asserted that} in \cref{fig:helixcomparison} the black helix winds at
...
\highlighttext{The next sections develop} slightly irregular edge lengths that support, for example,
...
Let $n1$ be a node on a  shortest edge between
two rails. Let $n2$ be a node on a different rail.
Increasing the difference $\|n2-n1\|$, increasing edge lengths $T(n_1,n_2)$
without bound,
...
\highlighttext{
\Cref{thm:pitchandparallel} justifies talking about the singular
\emph{pitch} of 
...
In the proof, below, \highlighttext{it is} useful to consider projection diagrams that are the axial projection
...
\begin{lemma}
  ...
  regular polygon of 3 or more sides, \highlighttext{otherwise} they fill in a complete circle.
  \label{lemma:ngon}
\end{lemma}
\begin{proof}
  All points lying on a helix projected along the axis lie on a circle in the $XY$-plane.
\highlighttext{(By \cref{lemma:notpoint}, the circle is not a simple point.)}
...
\begin{proof}
  ...
  term in the defintion of the helix, as in \cref{eq:colored}. \highlighttext{That the
  ...
  overspecific, in that the two circles are drawn of the same radius but \highlighttext{
    this proof does not depend on that.
  }
...
Now that \highlighttext{it is provent that the} axes are coincident and parallel and that the pitches
are the same for all helices, any optimum tetrahelix can
be generated with a \highlighttext{simple} equation for helices:
\begin{equation}
\label{eq:triple}
\vec{V}_{triple}(n,c) =
\left [
...
(the relationships are developed in \cref{eqrhoopt}) these are denoted with a $c$ subscript to show
...
\highlighttext{This equation does not completely explain} the  relationships beteween
...
\highlighttext{Observe} that when $\alpha = 0$, the helices are degenerate, having curvature of $0$,
...
However, \highlighttext{generating} the three helices with \cref{eq:triple}
\highlighttext{ will cause }
...
\end{theorem}
\begin{proof}
To prove this theorem, \highlighttext{one may} exhibit a symmetric tetrahelix (not yet shown to be optimal) which
...
radii of such a triple helix \highlighttext{has not yet been developed},
...
\highlighttext{one} can choose these values such that the three rail edges are of length $1$ and
...
tetrahelix generated by \cref{eq:triple}, \highlighttext{ and so by \cref{eq:optdef}} is not optimal.


\highlighttext{Although \cref{thm:eventhirds} which asserts} that a two-class tetrahelix is optimal
\highlighttext{has not yet been proven}, it suffices to show that there
...
Now that \highlighttext{is proven} that on any optimal tetrahelix the vertices
are on helices of the same axes and pitch, \highlighttext{observe} that the vertices 
...
\highlighttext{This allows }
the harder proof about where vertices occur along the $z$-axis.

\highlighttext{In fact,} the vertices must be distributed in even thirds
 along the $z$-axis as they are in the regular BC helix.
 \highlighttext{Note that it is already proven that} the rail
...
\begin{proof}
    ...
    \highlighttext{The minimax of $R(B)$ is greater than the minimax of $R(A)$} since there is a
...
input the ``rail angle'' $\rho$. \highlighttext{Define} $\rho$ to be the angle
...
parallel with the axis, or the ``twistiness'' of the tetrahelix. \highlighttext{Use }
the parameter $\chi = 1$ to indicate a chirality of counter-clockwise,
and $\chi = -1$ for clockwise. \highlighttext{Take} our coordinate system to be right-handed.
...
determine both of them together, so \highlighttext{one must} treat both as additional parameters.
...
\begin{proof}
By \cref{thm:eventhirds}, \highlighttext{one} can compute the (at most) three edge-lengths of an optimal
...
\highlighttext{one minimizes} the maximum edge-length ratio by choosing the largest radius
up until one-hop $= 1$, the rail-edge length. If \highlighttext{one} attempted to further increase the radius
\highlighttext{the tetrahelix} would not be optimal,
...
Therefore, \highlighttext{one decreases} the minimax length
of the whole system as the radius \highlighttext{increases}
...
\highlighttext{This leads to} a formula for $ d_{opt} $ computed from $\rho, r_{opt}$ via the rail angle equation \cref{railangle}:
...
\end{equation}
which \highlighttext{is expanded to:}
\begin{equation}
...
Thus, by computing $r_{opt}$  and $d_{opt}$ as a function of $\rho$ from this equation, \highlighttext{one} can
...
Since the axes are parallel, may define the \emph{inradius}, represented by the letter $i$, of a
...
From this equation, with the help of symbolic computation, \highlighttext{one may} observe that the inradius of the BC helix of unit rail length is $r_{in(\rho_{bc})} = \frac{3}{10\sqrt{2}} \approx 0.21$.
...
\begin{proof}
Choosing $d = 1$ and $\rho = 0$ \highlighttext{one can} use \cref{eqrhoopt} to find the radius of 
...
To the extent that \highlighttext{one values} tetrabeams (that is, tetrahelices with a rail angle of $0$,
and therefore zero curvature) as mathematical or engineering objects,
\highlighttext{this paper} has motivated the development of $\vec{H}_{general}$ as a transformation of $\vec{V}(n)$ defined by
...
\highlighttext{Observe} that  \cref{eqrhoopt,dopt} compute $r_{opt}$ and $d_{opt}$ which
...
even further does produce a clockwise tetrahelix, but one in which the
 ...
 a tetrahelix, but \highlighttext{its optimality has not proved or investigated} in that range.
...
solver, as \highlighttext{is done} on our website.
...
deriving  \cref{eqrhoopt}, \highlighttext{a continuum was created} by
