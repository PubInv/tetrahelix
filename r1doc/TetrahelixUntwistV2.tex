...
\title{\highlighttext{Transforming Optimal Tetrahelices} between the Boerdijk-Coxeter Helix and a Planar-faced Tetrahelix}
...
\Cref{fig:series} displays a continuum of tetrahelices optimal \highlighttext{in the sense of being
...
gives the vertices in space. Ideally, \highlighttext{this would be done}
...
these \highlighttext{continuous} rail helices have been rendered at a slightly higher radius than the vertices for clarity;
...
This \highlighttext{allows \cref{eq:colored} to be rewritten}:
...
Now that \highlighttext{it is proven that the} axes are coincident and parallel and that the pitches
...
\begin{proof}
    ...
    \highlighttext{The minimax of $B$ is greater than the minimax of $A$: $R(B) > R(A) $} since there is a
...
Since the axes are parallel, one may define the \emph{inradius}, represented by \highlighttext{$r_{in}$}, of a
...
even further does produce a clockwise tetrahelix, but one in which the
 ...
 a tetrahelix, but \highlighttext{its optimality has not been proved or investigated} in that range.
