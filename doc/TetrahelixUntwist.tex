...
The apparent rotation of a vertex an outer-edge, $\vec{V}(n)$ relative from $\vec{V}(n+3)$ for any integer $n$
...
\begin{proof}
  ...
  Let $\vec{B'}$ be the blue helix $\vec{B}$ translated in the $XY$-plane so that its axis is the $z$-axis and
  condicent with the red helix $R$. Let $D$ be the distance between the axis of the Blue helix
  $\vec{B}$ and $\vec{B'}$. We will show that if $D>0$ then $\vec{B}$ ``wobbles'' in a way that cannot be optimal.
  Define a wobble vector by:
  \begin{equation*}
    \vec{W}(n) = \vec{B}(n) - \vec{B}'(n)
  \end{equation*}
  where $\vec{B}(n)$ is the cartesian vector
  $\begin{bmatrix}
    x \\
    y 
  \end{bmatrix}$
  for the projection of the $n$th blue vertex.
  Note that $|| \vec{R}(n) - \vec{B'}(n+k)||$ is a constant for any $k$, because $R$ and $B'$ have the
...
\end{figure}
  ...
  The set of wobbles $\{\vec{W}(n) | \text{for any n}\}$ thus contains at least three vectors,
...
edges are slightly longer than the purple edges. In \ref{fig:naming}, which depicts the BC helix,
...
%% TODO: I have to add here whether I am using a left-handed or right-handed coordinate
%% system.
...
\begin{theorem}
  ...
  For rail angles of magnitude at most $\rho_{bc}$, tetrahelices generated by $\vec{H}_{general}$ are optimal in terms of minimum maximum ratio of member length when radius is chosen so that
...
\begin{align*}
  \text{rail} &= dist(\vec{H}_{general}(n,c,\rho,d_{\rho},r),\vec{H}_{general}(n+1,c,\rho,d_{\rho}),r)) = 1 \\
  \text{one-hop} &= dist(\vec{H}_{general}(n,c,\rho,d_{\rho},r),\vec{H}_{general}(n,c+1,\rho,d_{\rho},r))  \\
  \text{two-hop} &= dist(\vec{H}_{general}(n,c,\rho,d_{\rho},r),\vec{H}_{general}(n,c+2,\rho,d_{\rho},r))  \\  
\end{align*}
where $dist$ is the Cartesian distance function.


\begin{align*}
  \text{one-hop} &= dist(\vec{H}_{general}(n,c,\rho,d_{\rho}),\vec{H}_{general}(n,c+1,\rho,d_{\rho}),r)  \\
...
** Add the Mathematica notebook to the github repo (after cleaning up) and reference.
