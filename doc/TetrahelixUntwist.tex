...
design a column having the top and bottom tetrahedra in the same relationship to the capital and the basis to the
viewer.
...
\begin{proof}
    ...
    Therefore, any optimal tetrahelix has all one-hop edges between all rails equal to each other,
    all two-hop edges equal to each other, the $z$ distances between rails equal, and therefore
    $d/3$ from each other.
\end{proof}
...
the optimum radius, the one-hop length, and the two-hop length an engineer needs to
