...
\text{where:}
 ...
 $ \theta_{bc} = \arccos(-2/3) $  \text{ ,}\\
  \end{tabular}      
\end{equation}
and where $n$ represents each integer numbered node in succession on every colored rail.
...
\end{align*}
finally using algebra, we obtain
\begin{align*}
  c  ( \rho_{bc} +  2 \pi) /3  \text{.}
\end{align*}  
...
\begin{equation}
  \label{pitcheqn}
p(\rho) = \frac{2 \pi  \cdot d}{\rho} \text{ .}
\end{equation}
...
\end{align*}
This syntax just represents the Euclidean distance formula. Thus:
...
\begin{proof}
  Proof by our computer program that does this using Equation \eqref{eq:general} applied to the physical
  7-tet Tetrobot.
