...
\begin{abstract}
  ...
  A formula for computing the radius and z-axis travel that gives an optimal minimax tetrahelix for any rail angle
  between $0$ and $\rho_{bc}$ is given, thus defining a continuum of tetrahelices containing the equitetrabeam and
  BC helix.
...
From Robert Gray's site, repeating formula by H.S.M. Coxeter, for a counter-clockwise BC Helix:
\begin{equation}
  \label{graycoxeter}
V(n) =
\left [
...
in \eqref{graycoxer}, is $3 \theta - 2\pi$.
...
%% This graph demonstrates the derivative is always negative.
...
$\rho_{bc} \approx 35.43 \degree$, which gives the BC Helix.
 ...
 Because the equitetrabeam which as a rail angle of $0$ still has chirality, that is,
 one still must decide to connect with one-hop to the clockwise or counter-clockwise node,
 it is not poosible to build a smooth continuum where $\rho$ transitions from positive to
 negative which remains optimal. One can use a negative $\rho$ in $H_{general}$ but it does
 not produce minimax optimal tetrahelices. In other words, untwisting a conter-clockwise
 tetrahelix to $0$ and then going further does produce a clockwise tetrahelix, but one in which
 the one-hop and two-hop lengths in the wrong places (that is, two-hop becomes shorter than one-hop.)
...
Engineers has two fundamental mathamematical tools for constructing a tetrahelix.
They may use 
