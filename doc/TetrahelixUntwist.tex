...
\begin{figure}
  \centering
     \includegraphics[width=0.7\textwidth]{figures/HelixComparison.png}
     \caption{Rail helices vs. Coxeter/Gray helix}
...
\Cref{thm:cylinder} is not true when $\rho$ is an odd integer multiple of $\pi$.
...
\Cref{equitetrabeam} shows the equitetrabeam, which is defined in \cref{sec:equitetrabeam},
...
Writing as robotocists, the ``colored'' approach of thinking of three rails seems natural to us,
in part because our application
is focused on the edges, rather than any sort of interpretaion of the tetrahedra as solids.
\Cref{eq:general} is in terms of helices that are close
...
a generalization of the orginal Coxeter/Gray formulation which uses a single helix
to define all vertices, and a mathematician, chemist,
or solid modeler might prefer that. This formulation is equivalent to \Cref{eq:general},
but uses $\theta = \frac{\rho+ 2\pi}{3}$,
the rotation between rails. The same constraint on $r_{opt}$ and $d_{opt}$ holds if the user seeks minimax optimality.
...
more than the approach of \cref{eq:coxtergraygeneral}.
...
Improve the helix comparison by constructing by hand in the Inkscape.

Check the math on V_{general}.

Reread everything, especially the new parts.

Introduct the torsion concept.
