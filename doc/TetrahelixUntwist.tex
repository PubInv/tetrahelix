...
\left [
  \begin{tabular}{c}
   $ r_{\rho} \cos(\rho \kappa + s\cdot c 2 \pi /3) $\\
   $ r_{\rho}  \sin(\rho \kappa + s\cdot c 2 \pi /3) $\\
   $ d_{\rho} \kappa $
  \end{tabular}
  \right ],
\text{where:}
...
\end{align*}
Therefore, by computing $r_{opt}$ as a function of $\rho$ from this equation, we can construct minimax optimal tetrahelix for an $-\rho_{bc} \leq \rho \leq \rho_{bc}$.
