...
%% I have to add here whether I am using a left-handed or right-handed coordinate
%% system.
...
\]
But since $\text{two-hop}(r) >= \text{one-hop}(r)$, this is equivalent to:
\[
...
By inspection it seems likely that we will minize this set by equating $\text{one-hop}(r)$ or $\text{two-hop}(r)$
to 1, but to be absolutely sure we must examine the partial derivative of the ratio
...
Then:
\begin{align*}
  \frac{\text{two-hop}}{ \text{one-hop}}  &=
  \frac{\sqrt{\frac{4}{9}  + r_{\rho}^2(g_{\rho}+ j_{\rho})}}
...
radius $r_{\rho}$ is negative up until $\rho_{bc}$, this ratio goes down
as the radius goes up, and 
we optimize the overall minimax distance by choosing the largest radius
