...
\begin{abstract}
  ...
  The continuum allows a regular Tetrobot in the BC configuration to untwist into a hexapodal or $n$-podal robot
...
roboticist because the structure is an inherently rigid,
...
from which \cref{fig:closeup,fig:closeuporthog,fig:series,fig:naming,fig:equitetrabeam}
are taken.
...
\text{where:}
 ...
 $ \theta_{bc} = \arccos(-2/3) $  \text{,}\\
  \end{tabular}      
\end{equation}
and where $n$ represents each integer numbered vertex in succession on every colored rail.
...
generated by Equaiton \cref{graycoxeter},
...
Note in \cref{railanglefig} the $z$-axis travel for one rail edge is denoted by $d$. In Equaiton \cref{graycoxeter} and  \cref{eq:colored}, the variable
...
\begin{proof}
    ...
    all two-hop edges equal to each other, the $z$ distances between rails equal. Therefore vertices are
...
formed in the X,Y plane with origin $O$: $\angle \vec{H}(0,0) O \vec{H}(0,1)$ projecting out the $z$
...
$\vec{H}_{general}$ forces the user to select three values: $\rho$, $r_{\rho}$, and $d_{\rho}$ satisfying \cref{railangle}.
...
\begin{align*}
  \text{rail} &= || \vec{H}_{general}(n,c,\rho,d_{\rho},r) - \vec{H}_{general}(n+1,c,\rho,d_{\rho}),r)) || = 1 \text{ ,} \\
  \text{one-hop} &= || \vec{H}_{general}(n,c,\rho,d_{\rho},r) - \vec{H}_{general}(n,c+1,\rho,d_{\rho},r)) || \text{ and,} \\
  \text{two-hop} &= || \vec{H}_{general}(n,c,\rho,d_{\rho},r) - \vec{H}_{general}(n,c+2,\rho,d_{\rho},r)) || \text{ .}\\  
\end{align*}
...
\end{align*}
but: $  d_{\rho}^2 = 1 - 4 r^2 (\sin( \rho / 2)^2$, so we substitute:
\begin{align*}
  %% Formula above checks at sqrt(8/27), 1, 0, and rho_bc, h_bc, r_bc.  
...
\begin{align*}
  \text{two-hop}  &= \sqrt{\frac{4}{9} + r^2 (-\frac{16 (\sin^2( \rho / 2))}{9} + \sin^2(2\rho/3 + \frac{4\pi}{3})  + (1 - \cos(2\rho/3 + \frac{4\pi}{3}))^2)} \text{.}\\
\end{align*}
...
\begin{align*}
  1 &=  \sqrt{\frac{1}{9}  + r_{opt}^2(-\frac{4 (\sin^2( \rho / 2))}{9} + \sin^2(\rho/3+ \frac{2\pi}{3})  + (1 - \cos(\rho/3 + \frac{2\pi}{3}))^2)} \text{, so...} \\
...
even further does produce a clockwise tetrahelix, but one in which the
 ...
 The pitch of a helix for a fixed $z$-axis travel $d$
 is trivial (see \cref{pitcheqn}).
However, if one is computing 
$z$-axis travel from \cref{dopt} the pitch is not simple.
...
as the tetrobot is, they often used repeated tetrahedral cells, so this
work of pure geometry has some relevance to them.
...
distinct rails (the two-hop members), we can dynamically untwist a tetrobot
forming the Boerdijk-Coxeter configuration into the equitetrabeam which rests flat on the plane.

  Proof by our computer program that does this using  \cref{eq:general} applied to the physical
  7-tet Tetrobot.
