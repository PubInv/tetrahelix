...
of a tetrahelix onto the $XY$-plane. \Cref{fig:projectionDiagram.png} is an example of such a diagram.
\begin{lemma}
  ...
  regular polygon of 3 or more sides, else they form a circle.
  \label{lemma:ngon}
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  to some three helices, possibly with different radii, so long as there is a phase
  ...
  \end{equation*}
  where $\vec{B}(n)$ and $\vec{B'}$ is the cartesian vector
  $\begin{bmatrix}
    x \\
    y 
  \end{bmatrix}$
...
\begin{figure}[H] %float with two figures
  \centering
     ...
     \caption{The Equitetrabeam: Fully Untwisted Glussbot in Hexapod Configuration}
     \label{fig:glussbot}
\end{figure}
