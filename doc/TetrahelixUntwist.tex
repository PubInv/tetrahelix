...
\begin{align*}
  \text{one-hop} &= dist(H_{general}(n,c,\rho,h_{\rho}),H_{general}(n,c+1,\rho,d_{\rho}),r_{\rho})  \\
  \text{one-hop} &= dist(H_{general}(0,0,\rho,h_{\rho}),H_{general}(0,1,\rho,d_{\rho},r_{\rho}))  \\  
  \text{one-hop}  &= \sqrt{\frac{d_{\rho}^2}{9} + (,r_{\rho}\sin(\rho(3\cdot 0 + 0)) - r_{\rho}\sin(\rho(3\cdot 0 + 1)+\frac{2\pi}{3}))^2  +
    (r_{\rho}\cos(\rho(3\cdot0+0)) - r_{\rho}\cos(\rho(3\cdot 0+1) + \frac{2\pi}{3})))^2} \\
  \text{one-hop}  &= \sqrt{\frac{d_{\rho}^2}{9} + (0  - r_{\rho}\sin(\rho + \frac{2\pi}{3}))^2  +
    (r_{\rho} - r_{\rho}\cos(\rho + \frac{2\pi}{3}))^2} \\
  \text{one-hop}  &= \sqrt{\frac{d_{\rho}^2}{9} + r_{\rho}^2\sin^2(\rho + \frac{2\pi}{3})  +
    r_{\rho}^2(1 - \cos(\rho + \frac{2\pi}{3}))^2} \\
  \text{one-hop}  &= \sqrt{\frac{d_{\rho}^2}{9} + r_{\rho}^2(\sin^2(\rho + \frac{2\pi}{3})  + (1 - \cos(\rho + \frac{2\pi}{3}))^2)} \\
\end{align*}

By similar algebra and trigonometry:
\begin{align*}
  \text{two-hops}  &= \sqrt{\frac{4d_{\rho}^2}{9} + r_{\rho}^2(\sin^2(\rho + \frac{2\pi}{3})  + (1 - \cos(\rho + \frac{2\pi}{3}))^2)} \\
\end{align*}

Let $f_{\rho} = \sin^2(\rho + \frac{2\pi}{3})  + (1 - \cos(\rho + \frac{2\pi}{3})^2$.
...
we conclude that:
\begin{align*}
  \text{two-hops} - \text{one-hop}  &= \sqrt{\frac{4d_{\rho}^2}{9} + r_{\rho}^2f_{\rho}} - \sqrt{\frac{d_{\rho}^2}{9} + r_{\rho}^2f_{\rho}} 
\end{align*}

The analysis below here is incorrect because it doesn't take into account the change in $d_{\rho}$ with $\rho$. In the special case that
$\rho = 0$, the analysis is correct. Possibly I need to change to use two equations to try to find the optimal $\rho$ here.
In any case it must unify with the BC math!

Tomorrow try combining with our equation for $d_{\rho}$ to see if I can make it come out.
...
\begin{align*}
  1 &= \sqrt{\frac{d_{\rho}^2}{9} + r_{\rho}^2(\sin^2(\rho + \frac{2\pi}{3})  + (1 - \cos(\rho + \frac{2\pi}{3}))^2)} \\
  r_{\rho} &= \frac{\sqrt{9-d^2}  }{3\sqrt{\sqrt{3}\sin(\rho) +\cos(\rho) + 2}}
\end{align*}
(where $\sqrt{3}\sin(\rho) +\cos{\rho} + 2 \neq 0$), which does not occur when $\rho \leq \pi/3$).
