...
In Figure \ref{series}, the closest helix is the BC helix.
...
\begin{align*}
  ...
  d_{opt}^2 &= 1 - 4 \frac{4}{\frac{9}{2} \cdot ( \sqrt{3} sin(\rho/3) + \cos(\rho/3)) + cos(\rho)+ 8 } (\sin{ \rho / 2})^2   \\
  d_{opt}^2 &= 1 - \frac{32}{9 ( \sqrt{3} sin(\rho/3) + \cos(\rho/3)) + cos(\rho)+ 8 } (\sin{ \rho / 2})^2   \\
  d_{opt}^2 &= 1 - \frac{8 sin^2(\rho) }{ cos(r) + 9( \sqrt{3} sin(\rho/3) + \cos(\rho/3)) + 8 }   \\
    d_{opt} &= \sqrt{1 - \frac{8 sin^2(\rho) }{ cos(r) + 9( \sqrt{3} sin(\rho/3) + \cos(\rho/3)) + 8 }}    \numberthis  \label{dopt}  \\      
\end{align*}
...
curvature and torsion between the two. It seems most convenient to define this continuum in terms
of the rail angle $\rho$, where $0 \leq \rho \leq \rho_{bc}$.
...
\begin{align*}
\phi_{\lambda} &=  \lambda \cdot \rho_{bc}   \\
r_{\lambda}  &=  r_{opt}(\phi) \\
d_{\lambda} &=   d_{opt}(\phi) \\
\end{align*}
...
\end{figure}
 ...
 (Note: In fact there is something wrong with my math, negative lambda values do produces opposit chirality
...
where $\lambda \neq 0$:
\[
p(\lambda) = 2 \pi  \cdot d_{opt}
\]

There is thus an optimal continuum of tetrahelices untwisting between BC and equitetrabeam.
Perhaps surprisingly, this untwisting is accomplished only by changing the length of the two-hop member.
