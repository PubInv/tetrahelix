...
happens to be a tetrahelix, that has
the property that all rail edges are equal to all one-hop edges and all two-hop
edges are equal. Observe that although we have not get given the formula for the
radii of such a triple helix, we observe there are some values for $r$ and $\alpha$
for which this is true from this generic formula for a triple helix:
\[
\label{eq:triple}
V_{triple}(n,c) =
\left [
...
\begin{tabular}{c}
  ...
  \end{tabular}      
\]
In \cref{sec:parameterizing} we show how to find $r$ and $\alpha$.
...
a two-hop edge.  Therefore, a tetrahelix with different radii is not as some
tetrahelix generated by \cref{eq:triple}, and so it not optimal. (Note however,
that we have not yet exhibited a truly optimal tetrahelix, nor proven that such
a tetrahelix occurs when the one-hop edge, length is equal to the rail edge length,
which we do in \cref{sec:parameterizing}.)

Because an optimal tetrhelix has equivalent radii for all three helices,
it has equivalent rail edge lengths.
...
By \cref{eventhirds} we are justified renaming \cref{eq:triple} ``optimal'':
\[
V_{optimal}(n,c) =
\left [
  \begin{tabular}{c}
...
\begin{tabular}{c}
  ...
  \end{tabular}      
\]
where we have not yet investigated in the general case the relationships beteween
$\alpha$, $r$, and $d$ in this formulation.
