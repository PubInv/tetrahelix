...
Define the \emph{projected incircle radius} of a tetrahelix to be
the radius of the incircle of all point on the tetrahelix projected
down the $z$ axis into the $xy$ plane.
We call a tetrahelix
\emph{member-length minimax optimal} (or just \emph{optimal}) if there is not tetrahelix of the
same project incircle radius and pitch with a smaller maximum edge length difference.
...
unbounded length, so it is not optimal. So we are justified in talking about the pitch of
the tetrahelix, even though we think of it as three
helices. Similarly, if the axes are not parallel, there is an edge of
unbounded length in the structure, so it is not optimal. So all three
rails have the same rail length.

We wish to show that the three helices optimal tetrahelix also have
coincident axes.
...
Consider the projection along the $z$ axis of all vertices into the $xy$ plane, which will be
...
length $6r/sqrt{3}$.
This is a equivalent to some let of helices which do have their axes on
