...
\documentclass[10pt,final]{journals-1.0/asme2ej}
...
\lfoot{JMR-17-1184}
\cfoot{\thepage}           % page number in "outer" position of footer line
\rfoot{Read}
...
\Cref{fig:series} displays a continuum of tetrahelices optimal in the sense of being
...
If each static edge is replaced with an
...
for this to be possible, a moving robot named the Tetrobot is created as 
...
\begin{figure}
  \centering
     \includegraphics[width=0.65\textwidth]{figures/edge-diagram.png}
     \caption{Edge Naming}
  \label{fig:naming}
\end{figure}
...
\begin{corollary}
  The equitetrabeam with minimum maximum edge ratio is produced by $\vec{H}_{general}$ when $ r = \sqrt{\frac{8}{27}} $ and the longest member is $\frac{2}{\sqrt{3}}$,
...
No tables are used in this paper.

\section{Figure Caption List}

\begin{enumerate}
\item
  BC Helix Close-up (partly along axis)
\item
  BC Helix Close-up (orthogonal)
\item
  A Continuum of Tetrahelices
\item
  7-Tet Tetrobot in relaxed, or BC helix configuration
\item
  The Equitetrabeam: Fully Untwisted 7-Tet Tetrobot in Hexapod Configuration
\item
  Rail helices (H) vs. Coxeter/Gray helix (V)
\item
  Rail Angle Geometry
\item
  Wobble Vectors from Non-Coincident Axes
\item
  Edge Naming
\item
  Equitetrabeam
\item
  Partial Derivative against $\rho$ and $r$: $ \frac{\partial \frac{\text{two-hop}(r)}{ \text{one-hop}(r)}}{\partial \rho, r} $
\item
  Axial view of a BC-Helix
\item
  General One-hop Projection Diagram
\end{enumerate}
